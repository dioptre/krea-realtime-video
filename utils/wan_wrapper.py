...
class Wan2_2_VAEWrapper(torch.nn.Module):
    """Wrapper for Wan2.2 VAE model"""
    def __init__(self, vae_pth: Optional[str] = None):
        super().__init__()
        # Wan2.2 uses different normalization constants
        mean = [
        ...
        # init model - use provided path or default Wan2.1 VAE for now (Wan2.2 uses same VAE)
        if vae_pth is None:
            vae_pth = os.path.join(MODEL_FOLDER, "Wan2.1-T2V-1.3B", "Wan2.1_VAE.pth")
